...
import { HealthRepository, HEALTH_REPOSITORY } from './infrastructure/health.repository.interface';
...
export class HealthService {
  ...
  async testSmtp(email: string, message: string, clientIp: string) {
    ...
    if (nodeEnv !== Environment.Development && nodeEnv !== Environment.Test) {
      throw new BadRequestException('Este endpoint só está disponível em ambiente de desenvolvimento ou teste');
    }
